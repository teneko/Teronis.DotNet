...
namespace Teronis.Microsoft.JSInterop.LocalObject
{
    internal class JSLocalObjectActivatingInterceptor : IJSFunctionalObjectInterceptor
    {
        ...
        public void TryInvokeAsync<TValue>(ref JSFunctionalObjectInvocation<TValue> invocation)
        {
            if (invocation.Arguments.Length == 0 && jsLocalObjectType == invocation.GenericTaskArgumentType) {
                invocation.SetPromise((ValueTask<TValue>)(object)jsLocalObjectActivator.CreateLocalObjectAsync(invocation.Identifier));
            }
        }

        public void TryInvokeVoidAsync(ref JSFunctionalObjectInvocation invocation) { }
    }
}

...
namespace Redzen.Sorting
{
    ...
    public sealed partial class TimSort<T>
    {
        /// <summary>
        ...
        /// <param name="comparer">Comparer to be used to compare items.</param>
        private TimSort(int len, ref T[]? work, IComparer<T> comparer)
        {
        ...
        private void MergeLo(Span<T> s, int base1, int len1, int base2, int len2)
        {
            ...
            while (true) {
                ...
                do {
                    ...
                    if (LessThan(ref s[cursor2], ref tmp[cursor1])) {
                        s[dest++] = s[cursor2++];
                        count2++;
                        count1 = 0;
                        if (--len2 == 0) {
        ...
        private void MergeHi(Span<T> s, int base1, int len1, int base2, int len2)
        {
            ...
            while (true) {
                ...
                do {
                    ...
                    if (LessThan(ref tmp[cursor2], ref s[cursor1])) {
                        s[dest--] = s[cursor1--];
                        count1++;
                        count2 = 0;
                        if (--len1 == 0) {
        ...
        /// </remarks>
        private static void InsertionSort(Span<T> s, int start, IComparer<T> comparer)
        {
            if (start > 0)
                start--;
            ...
            for (int i = start; i < s.Length - 1; i++) {
                ...
                int j = i;
                while (j >= 0 && LessThan(ref k, ref s[j], comparer)) {
                    s[j + 1] = s[j];
                    j--;
                }
        ...
        private static int CountRunAndMakeAscending(Span<T> s, IComparer<T> comparer)
        {
            int runHi = 1;
            if (runHi == s.Length) {
                return 1;
            ...
            if (LessThan(ref s[runHi++], ref s[0], comparer)) {
                // Descending.
                while (runHi < s.Length && LessThan(ref s[runHi], ref s[runHi - 1], comparer)) {
                    runHi++;
                }
        ...
        /// <param name="work">An optional workspace array.</param>
        /// <param name="comparer">Comparer to be used to compare items.</param>
        public static void Sort(Span<T> span, ref T[]? work, IComparer<T> comparer)
        {
            if (span.Length < 2) {
            ...
            if (span.Length < MIN_MERGE) {
                int initRunLen = CountRunAndMakeAscending(span[lo..hi], comparer);
                InsertionSort(span, initRunLen, comparer);
                return;
            }
            ...
            TimSort<T> ts = new(span.Length, ref work, comparer);
            int minRun = MinRunLength(span.Length, MIN_MERGE);
            int nRemaining = span.Length;
            do {
                // Identify next run.
                int runLen = CountRunAndMakeAscending(span[lo..hi], comparer);

                // If run is short, extend to min(minRun, nRemaining).
                if (runLen < minRun) {
                    int force = nRemaining <= minRun ? nRemaining : minRun;
                    InsertionSort(span.Slice(lo, force), runLen, comparer);
                    runLen = force;
                }

...
namespace Teronis.Windows.Hooks
{
	...
	public abstract class GlobalHook
    {
        ...
        public GlobalHook(Assembly executingAssembly)
        {
            this.executingAssembly = executingAssembly;
            AppDomain.CurrentDomain.ProcessExit += new EventHandler(Application_ApplicationExit);
        }

        public static Func<IntPtr> GetMainWindowHandle => () => Marshal.GetHINSTANCE(Assembly.GetExecutingAssembly().GetModules()[0]);
        ...
        public void Start()
        {
            ...
            if (!_isStarted &&
                ...
                _handleToHook = SetWindowsHookEx(
                     _hookType,
                     _hookCallback,
                     Marshal.GetHINSTANCE(executingAssembly.GetModules()[0]),
                     0);
